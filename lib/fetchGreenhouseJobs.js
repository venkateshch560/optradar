...
function createJobHash(title, company, location, applyLink) {
  return crypto
    .createHash("sha256")
    .update(
      `${title}|${company}|${location}|${applyLink}`
        .toLowerCase()
        .trim()
    )
    .digest("hex");
...
function detectExperience(title) {
  ...
  return {
    level: "Experience not listed by employer",
    years: null,
  };
}
...
export async function fetchGreenhouseJobs() {
  ...
  for (const company of companies) {
    try {
      ...
      console.log(company, "jobs found:", jobs.length);
      ...
      for (const job of jobs) {
        ...
        const location =
          job.location?.name || "United States";

        if (!title || !applyLink) continue;

        const opt = classifyOpt(
      ...
      const recentEnough =
  Date.now() - new Date(postedAt).getTime() <=
  7 * 24 * 60 * 60 * 1000;
...
if (!recentEnough) continue;
      ...
      console.log(company, "rows prepared:", rows.length);
      ...
      if (uniqueRows.length > 0) {
        ...
        if (!error) {
  console.log(company, "inserted:", uniqueRows.length);
  totalSaved += uniqueRows.length;
} else {
  console.log("GREENHOUSE UPSERT ERROR:", company, error);
}
      }
    } catch (err) {
