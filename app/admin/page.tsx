...
export default async function AdminPage({
  ...
  if (params.key !== "myadminsecret123") {
    return (
    ...
    <main className="min-h-screen bg-[#050712] text-white">
      <div className="border-b border-white/10 bg-[#0B1020]">
        <div className="mx-auto max-w-7xl px-6 py-8">
          <p className="text-sm font-semibold text-blue-300">OPT Radar Admin</p>
          <h1 className="mt-2 text-5xl font-bold">User Analytics</h1>
          <p className="mt-3 text-gray-400">
            Track users, subscriptions, saved jobs, and applied activity.
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <div className="grid gap-5 md:grid-cols-4">
          ...
          <Card title="Active Paid Users" value={activeUsers} />
          <Card title="Inactive Users" value={inactiveUsers} />
          <Card title="Total Applied" value={appliedJobs.length} />
        </div>
              ...
              <tbody>
                {userRows.map((user: any) => (
                  <tr key={user.email} className="border-b border-white/5">
                    <td className="py-4 pr-4 font-medium">{user.email}</td>
                    ...
                    <td className="py-4 pr-4 text-gray-300">{user.status}</td>
                    <td className="py-4 pr-4">{user.savedCount}</td>
                    <td className="py-4 pr-4">{user.appliedCount}</td>
                ...
                {userRows.length === 0 && (
                  <tr>
                    <td className="py-8 text-gray-400" colSpan={6}>
                      No users yet.
                    </td>
                  </tr>
                )}
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          <Activity title="Recent Saved Activity" rows={savedJobs.slice(0, 8)} />
          <Activity title="Recent Applied Activity" rows={appliedJobs.slice(0, 8)} />
        </div>
      </div>
    </main>
  );
